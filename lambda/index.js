...
const { storeStatsInDb } = require("./scripts/utils/storeStatsInDb")

const als = require("./scripts/utils/asyncContext")

exports.handler = async (event, context) => {
    // Store the request ID in the AsyncLocalStorage
    // so it can be accessed by all the functions to store stats in the DB
    return als.run({ requestId: context.awsRequestId }, async () => {
        try {
            // If there are headers the request was made via HTTP
            // so we need to check if the api key is valid
            if (event.headers) {
                ...
                return {
                    statusCode: 202,
                    body: JSON.stringify({ message: `Accepted. Work triggered asynchronously. ${functionType}` }),
                }
