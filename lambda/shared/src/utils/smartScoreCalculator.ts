import { RawScore, SmartScoreOutput } from "@shared/types"
 ...
 *
 * @param rawScores - An array of raw score data for a user.
 * @param days - The number of previous days to consider (currently unused in the logic but kept for signature parity).
...
export function calculateSmartScore(
    rawScores: RawScore[],
    days: number,
): { smartScore: number; topBandDays: string[] } {
    if (!rawScores || rawScores.length === 0) {
        return { smartScore: 0, topBandDays: [] }
    }

    const enriched = rawScores.map((d) => ({
        ...d,
