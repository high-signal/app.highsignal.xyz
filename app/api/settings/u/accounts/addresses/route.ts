...
export async function POST(request: NextRequest) {
    try {
        ...
        // Only update name if it is being changed
        if (sanitizedFields.name !== undefined) {
            updateData.address_name = sanitizedFields.name
        }

        // Only update sharing settings if sharing is being changed
        if (changedFields.sharing !== undefined) {
            updateData.is_public = changedFields.sharing === "public"
        }
        ...
        // Only update sharing table if sharing settings are being changed
        if (changedFields.sharing !== undefined || changedFields.projectsSharedWith !== undefined) {
