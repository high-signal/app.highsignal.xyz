...
async function analyzeUserData(userData, username, maxValue, previousDays) {
  ...
  const basePrompt = `
    ...
    - ${Math.floor(
      maxValue * 0.1
    )}: Low-quality one-liners like "thanks" or "cool".
  ...
  // TODO: Increase limit
  const truncatedData =
    userDataString.substring(0, 10000) +
    (userDataString.length > 10000 ? "..." : "");
  console.log(
    `Using truncated data (${truncatedData.length} chars) for testing`
  );
