async function updateUserData(
  ...
  try {
    ...
    // Store the new last_updated date
    ...
    if (error) {
      ...
      console.log(
        `Successfully updated latestActivityDate in database for ${username}`
      );
    }
    ...
    if (signalError) {
      console.error(
        `Error storing signal strength for ${username}:`,
        signalError.message
      );
    } else {
      console.log(
        `Successfully stored signal strength response in database for ${username}`
      );
    }
  } catch (dbError) {
